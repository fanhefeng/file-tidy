...
export function executePlan(entries, { destDir, sourceDir }) {
  ...
  appendDuplicatesManifest(moved.filter((e) => e.action === 'duplicate'), destDir);
...
function appendDuplicatesManifest(dups, destDir) {
  ...
  const manifest = path.join(destDir, 'Duplicates', '清单.md');
  fs.mkdirSync(path.dirname(manifest), { recursive: true });
  const lines = [`\n## ${new Date().toLocaleString('zh-CN')}\n`];
  for (const d of dups) {
    lines.push(`- \`${path.basename(d.to)}\` 与已保留的 \`${d.keeperPath}\` 内容完全相同（SHA-256: ${d.hash.slice(0, 16)}…）`);
  }
